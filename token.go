...
func (mgr *WxTokenMgr) GetToken() string {
	...
	mgr.Set(KeyToken, newToken.Token, newToken.Expires)
...
func (mgr *RTokenMgr) GetToken() string {
	...
	// redis 就依赖expires
	mgr.session.Set(tokenKey, newToken.Token, time.Duration(newToken.Expires)*time.Second)
